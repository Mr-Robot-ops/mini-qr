...
export async function getLegacySvgString(
  ...
  const widthAttr = formatSvgNumber(width)
  const heightAttr = formatSvgNumber(height)
  const viewBox = `0 0 ${widthAttr} ${heightAttr}`

  const svgMarkup = [
    '<?xml version="1.0" encoding="UTF-8"?>',
    `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${widthAttr}" height="${heightAttr}" viewBox="${viewBox}" preserveAspectRatio="xMidYMid meet">`,
    `  <image x="0" y="0" width="${widthAttr}" height="${heightAttr}" preserveAspectRatio="xMidYMid meet" href="${dataUrl}" />`,
    '</svg>'
  ]
