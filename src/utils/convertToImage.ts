...
type OptionsWithPixelRatio = Options & { pixelRatio?: number }
...
const resolvePixelRatio = (candidate?: number): number => {
  if (typeof candidate === 'number') {
    return clampPixelRatio(candidate)
  }

  if (typeof window !== 'undefined') {
    const ratio = Number(window.devicePixelRatio)
    if (Number.isFinite(ratio) && ratio > 0) {
      return clampPixelRatio(ratio)
    }
  }
...
const sanitizeExportOptions = (options: Options): { pixelRatio: number; baseOptions: Options } => {
  const extended = options as OptionsWithPixelRatio
  const clone = { ...extended } as Record<string, unknown>
  delete clone.pixelRatio

  return {
    pixelRatio: resolvePixelRatio(extended.pixelRatio),
    baseOptions: clone as Options
  }
}
...
const getExportPreparation = (
  ...
  const { pixelRatio, baseOptions } = sanitizeExportOptions(options)
  const requestedWidth = parseDimension(baseOptions.width)
  const requestedHeight = parseDimension(baseOptions.height)
  ...
  if (requestedWidth && requestedHeight) {
    ...
    const captureScale = safeScale * pixelRatio
    ...
    const style: Record<string, string> = {
      ...(baseOptions.style ? (baseOptions.style as Record<string, string>) : {}),
    ...
    const captureWidth = Math.max(1, Math.round(requestedWidth * pixelRatio))
    const captureHeight = Math.max(1, Math.round(requestedHeight * pixelRatio))
  ...
  const captureWidth = Math.max(1, Math.round(targetWidth * pixelRatio))
  const captureHeight = Math.max(1, Math.round(targetHeight * pixelRatio))
...
const paintBlobWithRoundedCorners = async (
  ...
  new Promise((resolve, reject) => {
    ...
    image.onload = () => {
      const targetWidth = Math.max(1, Math.round(width || image.naturalWidth || image.width || 1))
      const targetHeight = Math.max(
        1,
        Math.round(height || image.naturalHeight || image.height || 1)
      )
    ...
    image.onerror = () => handleError('Failed to load image for rounding')

    const objectUrl = URL.createObjectURL(blob)
    image.src = objectUrl
    if (typeof image.decode === 'function') {
      image.decode().catch(() => {
        // Ignore decode errors and allow the load event to proceed.
      })
    }
  })
...
const getFormattedJpegOptions = (
  ...
  const { baseOptions } = sanitizeExportOptions(options)

  if (baseOptions.width && baseOptions.height) {
    const widthValue =
      parseDimension(baseOptions.width) ?? (element.offsetWidth || defaultOptions.width)
    const heightValue =
      parseDimension(baseOptions.height) ?? (element.offsetHeight || defaultOptions.height)
    const scale = getResizeScaleToFit(element, widthValue, heightValue) || 1
